...
namespace ngl_test_support
{
	namespace rt_dtl
	{
		inline bool can_use(std::string& acfg_root, std::string& acsv_root)
		{
			namespace fs = std::filesystem;

			std::error_code lec;
			const fs::path lcfg_root("./config");
			if (!fs::is_directory(lcfg_root, lec))
			{
				return false;
			}

			const fs::path lcfg_file = lcfg_root / "config" / "config.xml";
			if (!fs::is_regular_file(lcfg_file, lec))
			{
				return false;
			}

			const fs::path lcsv_root = lcfg_root / "csv";
			if (!fs::is_directory(lcsv_root, lec))
			{
				return false;
			}
			if (!fs::is_regular_file(lcsv_root / "tab_servers.csv", lec))
			{
				return false;
			}

			acfg_root = lcfg_root.string();
			acsv_root = lcsv_root.string();
			return true;
		}
	} // namespace rt_dtl
	...
	inline bool find_rt(std::string& acfg_root, std::string& acsv_root, std::string& aerr)
	{
		...
		if (rt_dtl::can_use(acfg_root, acsv_root))
		{
			aerr.clear();
			return true;
		}
		...
		aerr = std::format("missing ./config/config/config.xml or ./config/csv/tab_servers.csv from cwd={}", lcwd_txt);
		return false;
	}
		...
		int atcount = 1)
	{
		...
		if (!nconfig.load(lcfg_root, "config"))
		{
			aerr = std::format("nconfig.load failed: {}", nconfig.config_file());
			return false;
		}

		ngl::csv_base::set_path(lcsv_root);
