...
#conan install . --output-folder=build/Debug --build=missing -s build_type=Debug -c "tools.files.download:verify=False" -s compiler.cppstd=20
#conan install . --output-folder=build/Debug --build=missing -s build_type=Debug -c "tools.files.download:verify=False" -s compiler.cppstd=20 -o abseil/*:shared=False -o protobuf/*:shared=False
