...
*/
// File overview: Implements chat profanity filter and UTF-8 input validation.
...
namespace ngl::tools
{
	...
	std::vector<nacnode> filterword::m_nodes;
	int filterword::m_root = -1;

	void filterword::ensure_initialized()
	{
	...
	void filterword::advance_state(char c, int& cur)
	{
	...
	bool filterword::append_matches(int cur, int i, std::vector<std::pair<int, int>>& res)
	{
	...
	bool filterword::match_exists(char c, int& cur)
	{
		if (m_root < 0 || static_cast<std::size_t>(m_root) >= m_nodes.size())
		{
			return false;
	...
	void filterword::load(const std::string& apattern)
	{
		if (apattern.empty())
		{
			return;
	...
	bool filterword::match(char c, int& cur, int i, std::vector<std::pair<int, int>>& res)
	{
		if (m_root < 0 || static_cast<std::size_t>(m_root) >= m_nodes.size())
		{
			return false;
	...
	std::vector<std::pair<int, int>> filterword::match(const std::string& text)
	{
	...
	std::string filterword::filter(const std::string& text)
	{
	...
	bool filterword::utf8to32(const std::string& text1, std::u32string& text2)
	{
		try
		{
	...
	bool filterword::utf32to8(const std::u32string& text1, std::string& text2)
	{
		try
		{
	...
	bool filterword::is_filter(const std::string& text)
	{
		if (text.empty())
		{
			return false;
	...
	int32_t filterword::charcount(const std::u32string& astr)
	{
		int32_t lcount = 0;
	...
	const char* filterword::enfilter_message(filterword::enfilter astat)
	{
		if (tools::em<enfilter>::empty())
		{
			tools::em<enfilter>::set(enfilter_success, "成功");
			tools::em<enfilter>::set(enfilter_invalid_utf8, "失败:UTF-8 非法");
			tools::em<enfilter>::set(enfilter_emojispecial, "失败:存在特殊符号");
			tools::em<enfilter>::set(enfilter_charcount, "失败:字符个数不符合要求");
			tools::em<enfilter>::set(enfilter_filter, "失败:存在屏蔽字");
		}
		return tools::em<enfilter>::name(astat);
	}

	filterword::enfilter filterword::check_name(const std::string& astr, int32_t amincount, int32_t amaxcount)
	{
		std::u32string ltemp;
	...
	bool filterword::is_language_char(uint32_t codepoint)
	{
	...
	bool filterword::is_emojispecial(char32_t codepoint)
	{
	...
	bool filterword::is_emojispecial(const std::u32string& astr)
	{
	...
	void test_filterword()
	{
...

		{
			...
			{
				auto filtered = filterword::filter(text);
			...
			{// Whether
				...
				{
					filterword::enfilter lerror = filterword::check_name(lnames2[i], 1, 20);
					std::cout << lnames[i] << ":" << filterword::enfilter_message(lerror) << std::endl;
				}
			}
		}
	}
