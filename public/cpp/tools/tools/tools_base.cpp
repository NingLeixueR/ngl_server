...
namespace ngl
{
	...
	namespace tools
	{
		...
		std::string base64_encode(const char* adata, std::size_t alen)
		{
			...
			std::string ldst;
			ldst.resize(base_detail::base64_impl::enc_sz(alen));
			ldst.resize(base_detail::base64_impl::enc(ldst.data(), adata, alen));
			return ldst;
		}
		...
		std::string base64_decode(const char* adata, std::size_t alen)
		{
			...
			std::string ldst;
			ldst.resize(base_detail::base64_impl::dec_sz(alen));
			const auto lpair = base_detail::base64_impl::dec(ldst.data(), adata, alen);
