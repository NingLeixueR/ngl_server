...
namespace ngl::tools
{
	...
	int varint_length(parm<int64_t>& avalues)
	{
		...
		const uint64_t lval = varint_impl::zigzag64e(avalues.m_value);
	...
	int varint_length(parm<int32_t>& avalues)
	{
		...
		const uint32_t lval = varint_impl::zigzag32e(avalues.m_value);
	...
	bool varint_encode(varint_parm<int64_t>& aparm)
	{
		if (!sysconfig::varint())
		{
			return enc_base(aparm.m_buf, aparm.m_len, &aparm.m_value, 1, aparm.m_bytes);
		}
		...
		uint64_t lval = varint_impl::zigzag64e(aparm.m_value);
		int32_t lidx = 0;
		while (true)
		{
			if (lidx >= aparm.m_len || lidx >= varint_impl::g_64max)
			{
				return false;
			}
			if ((lval & ~0x7fULL) == 0)
	...
	bool varint_encode(varint_parm<int32_t>& aparm)
	{
		...
		uint32_t lval = varint_impl::zigzag32e(aparm.m_value);
		int32_t lidx = 0;
		while (true)
		{
			if (lidx >= aparm.m_len || lidx >= varint_impl::g_32max)
			{
				return false;
			}
			if ((lval & ~0x7fU) == 0)
	...
	bool varint_decode(varint_parm<int64_t>& aparm)
	{
		if (!sysconfig::varint())
		{
			return dec_base(aparm.m_buf, aparm.m_len, aparm.m_bytes, &aparm.m_value, 1);
		}
		if (aparm.m_buf == nullptr || aparm.m_bytes == nullptr || aparm.m_len <= 0)
		{
			return false;
		...
		*aparm.m_bytes += lidx + 1;
		aparm.m_value = varint_impl::zigzag64d(lraw);
		return true;
	}
	...
	bool varint_decode(varint_parm<int32_t>& aparm)
	{
		...
		*aparm.m_bytes += lidx + 1;
		aparm.m_value = varint_impl::zigzag32d(lraw);
		return true;
	}
	...
	bool to_bytes(void* apso, int alen, void* apto, int& aout_len)
	{
		...
		{
			if (!hex_byte(&static_cast<unsigned char*>(apso)[lidx * 2], &static_cast<unsigned char*>(apto)[lidx]))
			{
				return false;
			}
		}
