...
namespace ngl
{
	...
	{
		//// test 1 测试吞吐量
		time_wheel_config lconfig;
		ngl::time_wheel ltime(lconfig);
		int64_t lbegms = ltime.getms();
		int lcountsum = 0;
		...
		while (--lnum > 0)
		{
			for (int i = 0; i < 100000; ++i)
			{
				int lms = 1000;
				int lcount = 2;
				ltime.addtimer(wheel_parm
					{
						.m_ms = 1000,
						.m_intervalms = [](int64_t) {return 1000; } ,
						.m_count = 2,
						.m_fun = [&ltime, &lcountsum, lbegms](wheel_node* anode) {
							if (++lcountsum % 200000 == 0)
							{
								std::cout << (ltime.getms() - lbegms) << std::endl;
							}
						},
					});
			}
		...
		//// test 1 测试在回调过程中取消添加定时器
		wheel_parm lparm
		{
			.m_ms = 1 * 1000,
			.m_intervalms = [](int64_t) {return 1 * 1000; } ,
			.m_count = 60,
			.m_fun = [&ltime](ngl::wheel_node* anode)
			{
				std::cout << anode->m_parm.m_ms << ":" << ltime.getms() << ":1秒" << std::endl;
				wheel_parm lparm
				{
					.m_ms = 2 * 1000,
					.m_intervalms = [](int64_t) {return 2 * 1000; },
					.m_count = 60,
					.m_fun = [&ltime](ngl::wheel_node* anode)
					{
						std::cout << anode->m_parm.m_ms << ":" << ltime.getms() << ":####2秒" << std::endl;
						ltime.removetimer(anode->m_timerid);
					}
				};
			}
		};
		ltime.addtimer(lparm);
