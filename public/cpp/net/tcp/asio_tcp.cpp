...
*/
// File overview: Implements TCP accept loop, session creation, and data dispatch.
...
namespace ngl
{
		...
		m_service_ios(athread, etcp_buffmaxsize)
	{
		...
		{
			if (!ngl::tcp::is_optional_ipv6_acceptor_error(ex.code()))
			{
				throw;
			}
			log_warn()->print("asio_tcp ipv6 listener unavailable, continue with ipv4 only [{}]", ex.code().message());
			m_acceptor_v6.reset();
		}
		accept(true);
		accept(false);
				...
				if (ec)
				{
					...
					if (acount > 0)
					{
						...
						tools::wheel_parm lparm
						{
							...
							.m_count = 1,
							.m_fun = [this, aip, aport, afun, acount](const tools::wheel_node*)
							{
	...
	void  asio_tcp::start(const std::shared_ptr<service_tcp>& aservice)
	{
		...
		// This transport assumes process-lifetime ownership of asio_tcp.
