...
namespace ngl
{
	...
	std::shared_ptr<pack> actor_base::jsonpack(const std::string& apbname, const std::string& ajson, i64_actorid aactorid, i64_actorid arequestactorid)
	{
		...
		if (lmap.empty())
		{
            ...
            mkdef_map(pbnet::PROBUFF_NET_ROLE_NOT_CREATE);
            mkdef_map(pbnet::PROBUFF_NET_ROLE_CREATE);
            mkdef_map(pbnet::PROBUFF_NET_ROLE_CREATE_RESPONSE);
