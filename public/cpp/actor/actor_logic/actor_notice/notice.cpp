...
*/
// File overview: Implements notice data operations and notification lifecycle.
...
namespace ngl
{
	...
	void notice::remove_notice()
	{
		for (auto& lpair : data())
		{
			...
			if (tools::time::checkutc(lpnotice->mfinishtime()) == false)
			{
				log_error()->print(
					"remove_notice {}:{}-{}:{}"
					, lpnotice->mid()
					, tools::time::time2str(lpnotice->mstarttime(), "%Y-%m-%d %H:%M:%S")
					, tools::time::time2str(lpnotice->mfinishtime(), "%Y-%m-%d %H:%M:%S")
					, lpnotice->mnotice()
				);
				remove(lpnotice->mid());
			}
