...
*/
// File overview: Individual rank entry with score, player id, and snapshot data.
...
namespace ngl
{
	...
	struct rank_item
	{
		...
		std::map<pbdb::eranklist, rank_pair>	m_data;							// Attribute / time
		...
		void change(pbdb::eranklist atype, pbdb::db_ranklist& aranklist)
		{
			...
			rank_pair& lpair = m_data[atype];
			lpair.m_time = (int32_t)tools::time::gettime();
		...
		enum ecompare
		{
			eless_bigger	= 1,		// Translated comment.
			eless_less		= -1,		// Translated comment.
			eless_equal		= 0,		// Translated comment.
		};
