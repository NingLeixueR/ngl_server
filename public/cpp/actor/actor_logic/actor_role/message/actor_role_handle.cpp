...
*/
// File overview: Message handler registration and dispatch for the role actor.
		...
		if (handle_gm::empty())
		{
			...
			handle_gm::add("gmrechange") = [this](int id, ncjson& aos)
				{
					...
					if (!njson::pop(aos, { "data" }, lrechargeid))
					{
						return;
					}
					std::string lorder;
			...
			handle_gm::add("rechange") = [this](int id, ncjson& aos)
				{//actor_role::loginpay() callback
					...
					if (!njson::pop(aos, { "data" }, lrechange))
					{
						return;
					}
			...
			handle_gm::add("bantalk") = [this](int id, ncjson& aos)
				{
					...
					int32_t lduration = 0;
					if (!njson::pop(aos, { "data" }, lduration))
					{
						return;
					}
	...
	bool actor_role::handle([[maybe_unused]] const message<pbnet::PROBUFF_NET_CMD>& adata)
	{
		...
		if (handle_cmd::empty())
		{
			...
			handle_cmd::add("/chat") = []([[maybe_unused]] const std::shared_ptr<pack>& apack, actor_role* role, [[maybe_unused]] const char* aparm)
				{
					// C 3 0 channelid // getallchatrecord
					// C 3 1 channelid "xxxx" // chat
					std::vector<std::string> lvec;
