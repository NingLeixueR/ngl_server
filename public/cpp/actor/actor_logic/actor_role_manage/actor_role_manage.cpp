...
namespace ngl
{
		...
		actor(
			actorparm
			{
				.m_parm
				{
					...
					.m_id = ttab_servers::instance().tab()->m_tcount
				},
				.m_weight = 0x7fffffff,
			})
	{
	...
	{
		return nguid::make(actor_type(), nguid::none_area(), ttab_servers::instance().tab()->m_tcount);
	}
	...
	void actor_role_manage::nregister()
	{
		...
		// 绑定自定义np_消息
		register_handle_custom<actor_role_manage, false>::func<
			mforward<np_gm>
		>(false);

		// 绑定pb消息
		register_handle_proto<actor_role_manage, false>::func<
