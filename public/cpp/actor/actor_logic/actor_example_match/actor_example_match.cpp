...
namespace ngl
{
	...
	void actor_example_match::sync_match_info(room* aroom, i64_actorid aroleid /*= nguid::make()*/)
	{
		...
		{
			const pbdb::db_brief* lpbrief = tdb_brief::nsp_cli<actor_friends>::getInstance(id_guid()).getconst(id);
			if (lpbrief != nullptr)
			{
	...
	{
		const pbdb::db_brief* lpbrief = tdb_brief::nsp_cli<actor_friends>::getInstance(id_guid()).getconst(aroleid);
		if (lpbrief == nullptr)
		{
			return nullptr;
		}
