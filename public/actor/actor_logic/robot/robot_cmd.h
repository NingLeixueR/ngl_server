...
namespace ngl
{
	...
	public:
		...
		static bool parse_command(std::vector<std::string>& aparm)
		{
			...
			i64_actorid lid = ngl::actor_guid::make(ACTOR_MANAGE_ROBOT, tab_self_area, nconfig::m_nodeid);
			handle_pram lparm;
			ngl::handle_pram::create<robot_pram, false, false>(lparm, lid, actor_guid::moreactor(), ldata);
